...
def getData(path, output_path):
    imgs = os.listdir(path)
    result = []
    for img in imgs:
        frame = cv2.imread(path + img)
        vec = getFaceCoordinate(frame)
        if len(vec) == 136:
            result.append(vec)
...
getData('D:\\CodingProject\\PyCharmProject\\CIS-CV\\dataset\\expdataset\\Neutral\\',
        'data/neutral.csv')
getData('D:\\CodingProject\\PyCharmProject\\CIS-CV\\dataset\\expdataset\\Happy\\',
        'data/happy.csv')
