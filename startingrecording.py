...
save_video_path = 'supervision/records/'+location+'_datetime.avi'.replace('datetime', time.strftime('%Y%m%d-%H%M%S',
                                        time.localtime(time.time())))

# 启动画面监控程序
record_video_command = 'curl -X POST http://%s:%d/record_status-d "status=true&save_video_path=%s"'%(camera_host_ip, camera_host_port,
                                 save_video_path)
...
# 主程序彻底结束之前，停止录像
stop_record_video_command =\
    'curl -X POST http://%s:%d/record_status-d "status=false"'\
